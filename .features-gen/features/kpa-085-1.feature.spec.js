// Generated from: features/kpa-085-1.feature
import { test } from "../../steps/fixtures.ts";

test.describe('KPA-085-1 대여권 환불 검증 (이용권 내역에서)', () => {

  test('대여권 환불 검증 (이용권 내역에서)', async ({ Given, When, Then, And, ai, loginPage, page }) => { 
    ...
    await And('충전 내역 탭에서 구매한 대여권 영역을 클릭한다', null, { ai, loginPage, page }); 
    await And('사용자가 좌측 작품 정보 영역 이미지 하단의 대여권 n장 소장권 n장 > 영역에서 > 아이콘을 클릭한다.', null, { ai, loginPage, page }); 
    await And('이용권 내역 팝업창에서 구매를 취소할 대여권 타입을 클릭한다', null, { ai, loginPage, page }); 
