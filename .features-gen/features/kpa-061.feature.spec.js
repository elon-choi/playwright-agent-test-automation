...
test.describe('KPA-061 시나리오 검증', () => {
  ...
  test('오늘신작 메뉴 및 작품 홈 이동 검증', async ({ Given, When, Then, And, ai, loginPage, page }) => { 
    ...
    await When('사용자가 상단의 추천 GNB 메뉴를 클릭한다', null, { ai, page }); 
    await And('사용자가 오늘신작 서브탭을 클릭한다', null, { ai, page }); 
    await Then('오늘신작 메뉴 하단에 다음 요소들이 노출된다:', {"dataTable":{"rows":[{"cells":[{"value":"요소명"}]},{"cells":[{"value":"웹툰 탭"}]},{"cells":[{"value":"웹소설 탭"}]},{"cells":[{"value":"책 탭"}]},{"cells":[{"value":"TODAY 섹션"}]}]}}, { page }); 
    await When('사용자가 첫번째 신작 작품을 클릭한다', null, { ai, page }); 
    await And('사용자는 클릭한 작품의 작품홈으로 이동한다', null, { page }); 
  });
