...
test.describe('KPA-032 시나리오 검증', () => {
  ...
  test('최근본 작품 정렬 및 실시간 랭킹 작품 감상 검증', async ({ Given, When, Then, And, ai, loginPage, page }) => { 
    ...
    await And('사용자가 "메인홈" > "웹툰" > "실시간 랭킹" > 1위 작품을 클릭하고, 해당 작품의 1회차를 클릭한다', null, { ai, loginPage, page }); 
    ...
    await And('실시간 랭킹 1위 작품의 1회차가 정상적으로 감상된다', null, { ai, loginPage, page }); 
    await And('"최근본 작품" 탭의 최상단에 사용자가 3번에서 감상한 작품의 이력이 노출된다', null, { ai, loginPage, page }); 
  });
