...
test.describe('KPA-048 시나리오 검증', () => {
  ...
  test('운영 중인 배너의 UI 및 동작 검증', async ({ Given, When, Then, And, ai, loginPage, page }) => { 
    ...
    await And('배너 영역의 다음 화살표 버튼을 클릭하여 배너가 변경됨을 확인한다', null, { ai, page }); 
    await And('현재 노출된 운영 배너의 링크 정보를 저장하고 클릭한다', null, { ai, page }); 
    await Then('저장된 링크 주소로 페이지가 이동하였는지 확인한다', null, { page }); 
    await And('운영 배너가 노출된다'); 
