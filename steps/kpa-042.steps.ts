...
And("메인홈에서 웹툰 > 실시간 랭킹 > 1위 작품을 클릭하여 이용권을 구매한다", async ({ page }) => {
  await page.goto(getBaseUrlOrigin(), { waitUntil: "domcontentloaded", timeout: 10000 });
  await page.waitForTimeout(500);
  ...
  if (await buy.count() > 0) await buy.first().click({ timeout: 5000 });
  await page.waitForTimeout(500);
});
...
Then("구매 작품이 정렬 기준에 따라 올바르게 노출된다", async ({ page }) => {
  await expect(page.locator('a[href*="/content/"]').first()).toBeVisible({ timeout: 5000 });
});
