// Feature: KPA-121 - 무료 회차에서 다음 회차로 이동
// 전체 연령/무료 회차/최하단 스크롤은 common, Then "다음 회차로 이동"은 common.episode.steps.ts
...
And("사용자가 \"다음화 보기 >\" 버튼을 클릭한다", async ({ page }) => {
  const nextBtn = page.getByRole("button", { name: /다음화\s*보기|다음\s*화\s*보기/i })
    .or(page.getByRole("link", { name: /다음화\s*보기|다음\s*화\s*보기|다음\s*회차/i }))
    .or(page.getByText(/다음화\s*보기\s*>|다음\s*화\s*보기|다음\s*회차/i).first());
