...
And("사용자가 {string} > {string} > {string}에서 1위~20위 중 임의의 한 작품을 클릭하고, 해당 작품의 1회차를 감상한다", async ({ page }, _1: string, _2: string, _3: string) => {
  ...
  // "첫 화 보기" 버튼 우선 시도
  const firstViewBtn = page.getByRole("button", { name: /첫\s*화\s*보기/i })
    .or(page.getByText(/첫\s*화\s*보기/i));
  if ((await firstViewBtn.count()) > 0 && (await firstViewBtn.first().isVisible().catch(() => false))) {
    await firstViewBtn.first().click({ timeout: 8000 });
  } else {
    // 회차 목록에서 1화 텍스트 또는 뷰어 링크 클릭
    const firstEpisode = page.getByText(/1화/).first();
...
And("선택한 작품의 1회차가 정상적으로 감상된다", async ({ page }) => {
  // 뷰어 진입 또는 작품 상세 페이지(무료 회차 클릭 후 팝업 등) 확인
  await page.waitForURL(/\/viewer\/|\/content\//i, { timeout: 10000 }).catch(() => null);
  const isViewer = /\/viewer\//i.test(page.url());
  const isContent = /\/content\//i.test(page.url());
  expect(isViewer || isContent, `뷰어 또는 작품 상세 페이지에 있어야 합니다. 현재: ${page.url()}`).toBe(true);
});
