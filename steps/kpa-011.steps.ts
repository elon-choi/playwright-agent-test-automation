...
When("사용자가 임의의 기다무 BM 작품 카드를 클릭한다", async ({ page }) => {
  const candidates = [
...
When("사용자가 회차탭 하단의 임의의 기다무 회차를 클릭한다", async ({ page }) => {
...
When("사용자가 작품 이미지 좌측 상단에 3다무 또는 기다무 뱃지가 있는 작품 카드를 클릭한다", async ({ page }) => {
...
When("사용자가 홈 탭 하단의 회차 리스트에서 동일한 뱃지가 있는 기다무 회차를 클릭한다", async ({ page }) => {
  await page.waitForTimeout(600);
