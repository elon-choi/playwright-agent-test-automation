...
And("사용자가 좌측 작품 정보 영역 이미지 하단의 대여권 n장 소장권 n장 > 영역에서 > 아이콘을 클릭한다.", async ({ page }) => {
  ...
  // 대여권 n장 + 소장권 n장이 함께 보이는 cursor-pointer 블록 찾기
  const blockCandidates = [
    page.locator('[class*="cursor-pointer"], div.cursor-pointer').filter({ hasText: /대여권\s*\d+\s*장/ }).filter({ hasText: /소장권\s*\d+\s*장/ }).filter({ hasNotText: /이어보기/ }),
    page.locator('div').filter({ hasText: /대여권\s*\d+\s*장/ }).filter({ hasText: /소장권\s*\d+\s*장/ }).filter({ hasNotText: /이어보기|전체\s*\d+/ }),
  ];
  let block: any = null;
  for (const candidate of blockCandidates) {
    const cnt = await candidate.count().catch(() => 0);
    if (cnt > 0) {
      // 가장 좁은(innermost) 요소를 선택 — 넓은 컨테이너 클릭 방지
      for (let i = 0; i < cnt; i++) {
        const el = candidate.nth(i);
        if (await el.isVisible().catch(() => false)) { block = el; break; }
      }
      if (block) break;
    }
  }
  ...
  // 팝업 윈도우 캡처 준비 (이용권 내역은 별도 브라우저 윈도우로 열림)
  const popupPromise = page.waitForEvent("popup", { timeout: 12000 }).catch(() => null);

  // img[alt="더보기"] 또는 블록 내 img(> 화살표) 또는 블록 자체 클릭
  ...
  if ((await arrow.count().catch(() => 0)) > 0 && (await arrow.isVisible().catch(() => false))) {
    ...
    const arrowImg = block.locator('img').first();
    if ((await arrowImg.count().catch(() => 0)) > 0 && (await arrowImg.isVisible().catch(() => false))) {
      await arrowImg.click({ timeout: 8000, force: true });
    } else {
  ...
  if (popup && typeof popup.isClosed === "function" && !popup.isClosed()) {
    ...
    console.log("[KPA-085] 이용권 내역 팝업 윈도우 캡처 성공");
  } else {
    // 팝업이 아닌 오버레이 방식일 수도 있음 — 메인 페이지에서 대기
    console.log("[KPA-085] 팝업 윈도우 없음, 오버레이 확인 시도");
    await page.getByText(/이용권\s*내역/).filter({ hasNotText: /메뉴/ }).first().waitFor({ state: "visible", timeout: 8000 }).catch(() => null);
  }
  await safeWait(page, 1500);
});
...
async function tryClickCancellable대여권Type(_scope: any, page: any): Promise<boolean> {
  if (isPageClosed(page)) return false;
  // 이용권 내역 팝업 윈도우 내 "대여권 n장 >" 클릭 가능 영역 클릭 → 상세 진입
  // 실제 DOM: div.cursor-pointer.items-center > span.font-small1("1장") + img[alt="다음"]

  const popupPage = getTicketPopupPage(page);
  const target = popupPage;

  const popupHeader = target.getByText(/이용권\s*내역/).first();
  if (!(await popupHeader.isVisible().catch(() => false))) {
    console.log("[KPA-085 대여권] 팝업 헤더 미발견, target===page:", target === page);
    return false;
  }

  // img[alt="다음"]을 포함하는 cursor-pointer div 찾기 (hasText 제거 — 공백 문제 방지)
  const clickableAreas = target.locator('div.cursor-pointer, [class*="cursor-pointer"]')
    .filter({ has: target.locator('img[alt="다음"]') });
  const cnt = await clickableAreas.count().catch(() => 0);
  console.log(`[KPA-085 대여권] cursor-pointer+다음 영역: ${cnt}개`);

  for (let i = 0; i < cnt; i++) {
    if (isPageClosed(page)) return false;
    if (isPageClosed(target)) return false;
    const area = clickableAreas.nth(i);
    if (!(await area.isVisible().catch(() => false))) continue;

    // evaluate로 (1) n장 텍스트 확인, (2) 가장 가까운 행이 대여권인지 확인
    const info = await area.evaluate((el: HTMLElement) => {
      const ownText = (el.textContent || "").trim();
      const hasN장 = /[1-9]\d*장/.test(ownText);
      // 부모를 5단계까지 올라가며 "대여권" 텍스트가 있으면서 "소장권"이 없는 행 찾기
      // 단, 가장 가까운(narrow) 매칭을 우선 사용
      let node: HTMLElement | null = el.parentElement;
      let nearestType = "unknown";
      for (let level = 0; level < 5 && node; level++) {
        const text = node.innerText || "";
        const has대 = /대여권/.test(text);
        const has소 = /소장권/.test(text);
        if (has대 && !has소) { nearestType = "대여권"; break; }
        if (has소 && !has대) { nearestType = "소장권"; break; }
        if (has대 && has소) { nearestType = "both"; break; }
        node = node.parentElement;
      }
      return { hasN장, nearestType, ownText: ownText.substring(0, 30) };
    }).catch(() => ({ hasN장: false, nearestType: "error", ownText: "" }));

    console.log(`[KPA-085 대여권] 영역 ${i}: hasN장=${info.hasN장}, type=${info.nearestType}, text="${info.ownText}"`);

    if (!info.hasN장) continue;
    // "대여권" 또는 "both" (행 구분이 넓은 경우) 허용, "소장권" 전용 행은 제외
    if (info.nearestType === "소장권") continue;

    console.log(`[KPA-085 대여권] "n장 >" 영역 ${i} 클릭 시도`);
    await area.scrollIntoViewIfNeeded().catch(() => null);
    await safeWait(page, 300);
    try {
      await area.click({ timeout: 8000 });
    } catch (e) {
      console.log(`[KPA-085 대여권] 클릭 실패:`, String(e).substring(0, 100));
      continue;
    }
    await safeWait(page, 2500);

    // 상세 진입 확인: 팝업 + 메인 페이지 모두 확인
    const checkTarget = !isPageClosed(target) ? target : page;
    let hasDetail = await checkTarget.getByText(/구매\s*취소하기|취소\s*가능한\s*이용권|이용권\s*내역\s*상세/).first().isVisible().catch(() => false);
    if (!hasDetail && checkTarget !== page) {
      hasDetail = await page.getByText(/구매\s*취소하기|취소\s*가능한\s*이용권|이용권\s*내역\s*상세/).first().isVisible().catch(() => false);
    }
    const detailUrl = (() => { try { return checkTarget.url(); } catch { return ""; } })();
    if (!hasDetail) hasDetail = /\/history\/ticket\/detail/i.test(detailUrl);
    console.log(`[KPA-085 대여권] hasDetail=${hasDetail}, url=${detailUrl.substring(0, 60)}`);
    if (hasDetail) return true;
  }
  return false;
}

async function tryClickN장ArrowInScope(_scope: any, page: any): Promise<boolean> {
  if (isPageClosed(page)) return false;
  const target = getTicketPopupPage(page);
  // div.cursor-pointer > img[alt="다음"] + span("n장") 클릭 가능 영역
  const clickableAreas = target.locator('div.cursor-pointer, [class*="cursor-pointer"]')
    .filter({ has: target.locator('img[alt="다음"]') });
  const cnt = await clickableAreas.count().catch(() => 0);
  for (let i = 0; i < cnt && i < 5; i++) {
    ...
    if (!(await area.isVisible().catch(() => false))) continue;
    // n장 텍스트가 있는지 확인
    const ownText = await area.evaluate((el: HTMLElement) => (el.textContent || "").trim()).catch(() => "");
    if (!/[1-9]\d*장/.test(ownText)) continue;
    await area.scrollIntoViewIfNeeded().catch(() => null);
    await safeWait(page, 300);
    try {
    ...
    await safeWait(page, 1500);
    const checkTarget = !isPageClosed(target) ? target : page;
    const hasDetail = await checkTarget.getByText(/구매\s*취소|취소\s*가능한\s*이용권|이용권\s*내역\s*상세/).first().isVisible().catch(() => false);
    if (hasDetail || /\/history\/ticket\/detail/i.test((() => { try { return checkTarget.url(); } catch { return ""; } })())) return true;
  }
  return false;
}
...
async function reenterTicketHistoryAndOpenDetail(page: any, loginPage: any, ticketType: "대여권" | "소장권", rowIndex: number = 0): Promise<boolean> {
  ...
  if ((await block.count().catch(() => 0)) > 0 && (await block.isVisible().catch(() => false))) {
    ...
    if (newPopup && typeof newPopup.isClosed === "function" && !newPopup.isClosed()) {
      (page as any).__ticketPopup = newPopup;
      await newPopup.getByText(/이용권\s*내역|대여권|소장권/).first().waitFor({ state: "visible", timeout: 8000 }).catch(() => null);
      console.log("[KPA-085 reenter] 이용권 내역 팝업 윈도우 재캡처 성공");
    } else {
      console.log("[KPA-085 reenter] 팝업 윈도우 캡처 실패");
    }
  }
  await safeWait(page, 1500);
  return true;
...
async function tryClickCancellable소장권Type(_scope: any, page: any): Promise<boolean> {
  if (isPageClosed(page)) return false;
  ...
  const popupHeader = target.getByText(/이용권\s*내역/).first();
  if (!(await popupHeader.isVisible().catch(() => false))) return false;

  // img[alt="다음"]을 포함하는 cursor-pointer div 찾기 (hasText 제거 — 공백 문제 방지)
  const clickableAreas = target.locator('div.cursor-pointer, [class*="cursor-pointer"]')
    .filter({ has: target.locator('img[alt="다음"]') });
  const cnt = await clickableAreas.count().catch(() => 0);
  console.log(`[KPA-085 소장권] cursor-pointer+다음 영역: ${cnt}개`);
  ...
  for (let i = 0; i < cnt; i++) {
    ...
    // evaluate로 (1) n장 텍스트 확인, (2) 가장 가까운 행이 소장권인지 확인
    const info = await area.evaluate((el: HTMLElement) => {
      const ownText = (el.textContent || "").trim();
      const hasN장 = /[1-9]\d*장/.test(ownText);
      let node: HTMLElement | null = el.parentElement;
    ...
    console.log(`[KPA-085 소장권] 영역 ${i}: hasN장=${info.hasN장}, type=${info.nearestType}, text="${info.ownText}"`);

    if (!info.hasN장) continue;
    // "소장권" 또는 "both" 허용, "대여권" 전용 행은 제외
    if (info.nearestType === "대여권") continue;

    console.log(`[KPA-085 소장권] "n장 >" 영역 ${i} 클릭 시도`);
    await area.scrollIntoViewIfNeeded().catch(() => null);
    await safeWait(page, 300);
    try {
      await area.click({ timeout: 8000 });
    } catch (e) {
      console.log(`[KPA-085 소장권] 클릭 실패:`, String(e).substring(0, 100));
      continue;
    }
    await safeWait(page, 2500);
    ...
    const detailUrl = (() => { try { return checkTarget.url(); } catch { return ""; } })();
    if (!hasDetail) hasDetail = /\/history\/ticket\/detail/i.test(detailUrl);
    console.log(`[KPA-085 소장권] hasDetail=${hasDetail}, url=${detailUrl.substring(0, 60)}`);
    if (hasDetail) return true;
  }
  return false;
}
