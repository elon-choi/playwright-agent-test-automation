...
And("구매작품 리스트에 선택한 작품이 더 이상 노출되지 않는다", async ({ page }) => {
  ...
  if (selectedContentHref) {
    ...
    // selectedContentHref가 없으면 삭제 완료 토스트 또는 편집 모드 유지 확인
    const toast = page.getByText(/삭제\s*되었|삭제\s*완료|항목.*삭제/i).first();
    const editPage = page.getByText(/구매\s*작품\s*편집|편집/i).first();
    await expect(toast.or(editPage)).toBeVisible({ timeout: 5000 });
  }
});
