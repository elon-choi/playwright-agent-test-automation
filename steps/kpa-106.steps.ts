...
When("사용자가 [답글] 버튼을 클릭하고 임의의 답글을 입력한다", async ({ page }) => {
  ...
  if ((await replyBtn.count()) > 0) {
    await replyBtn.click({ timeout: 6000 }).catch(() => null);
    await page.waitForTimeout(400);
  }
  const input = page.getByRole("textbox").or(page.locator("textarea").or(page.locator('input[type="text"]'))).first();
  if ((await input.count()) > 0) await input.fill("테스트 답글입니다.", { timeout: 5000 }).catch(() => null);
...
And("사용자가 [답글 등록] 버튼을 클릭한다", async ({ page }) => {
  ...
  if ((await submitBtn.count()) > 0) await submitBtn.click({ timeout: 6000 }).catch(() => null);
