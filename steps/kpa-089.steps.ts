...
Then("구매회차 목록에 사용자가 구매한 회차 리스트가 노출된다", async ({ page }) => {
  ...
  if (noPurchaseVisible) {
    return;
  }
  const hasListOrSection = await page.getByText(/구매\s*회차|회차\s*목록|회차/i).first().isVisible().catch(() => false);
  expect(hasListOrSection, '구매회차 체크박스 옵션 체크 상태에서 구매 회차 목록 또는 "구매한 회차가 없습니다." 메시지가 노출되어야 합니다.').toBe(true);
});