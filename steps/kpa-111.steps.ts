...
Then("이미지 여백이 흰색으로 변경된다.", async ({ page }) => {
  ...
  const bgColor = await page.evaluate(() => {
    const viewer = document.querySelector('[class*="viewer"], [class*="Viewer"], main, #root > div');
    if (!viewer) return "";
    return window.getComputedStyle(viewer).backgroundColor;
  });
  const isWhitish = /rgb\(\s*255,\s*255,\s*255\s*\)|rgba\(\s*255,\s*255,\s*255|white/i.test(bgColor);
  const hasWhiteClass = (await page.locator('[class*="white"], [class*="White"], [data-theme*="white"]').count()) > 0;
  expect(isWhitish || hasWhiteClass, `여백이 흰색이어야 합니다. 배경색: ${bgColor}`).toBe(true);
});
...
Then("이미지 여백이 검정으로 변경된다.", async ({ page }) => {
  ...
  const isBlackish = /rgb\(\s*0,\s*0,\s*0\s*\)|rgba\(\s*0,\s*0,\s*0|black/i.test(bgColor);
  const hasBlackClass = (await page.locator('[class*="black"], [class*="Black"], [data-theme*="black"]').count()) > 0;
  expect(isBlackish || hasBlackClass, `여백이 검정이어야 합니다. 배경색: ${bgColor}`).toBe(true);
});
