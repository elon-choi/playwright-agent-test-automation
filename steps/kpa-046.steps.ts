...
When("사용자가 임의의 작품 리스트에서 선물 받기 메뉴를 클릭한다", async ({ page }) => {
  ...
  if (await btn.count() > 0) {
    ...
    // 버튼이 존재해도 실제 선물이 없을 수 있음 — 토스트로 실제 수령 여부 판정
    const toast = page.getByText(/지급|이용권.*지급|선물.*완료/i).first();
    giftReceived = await toast.isVisible({ timeout: 3000 }).catch(() => false);
  }
...
Then("사용자는 선물함 화면에 진입하며, 다음과 같은 메뉴가 노출된다", async ({ page }) => {
  await expect(page.getByText(/선물함|전체|웹툰|웹소설|책/i).first()).toBeVisible({ timeout: 8000 });
});

Then("클릭한 작품의 이용권이 지급되었다는 토스트 메시지가 노출된다", async ({ page }) => {
  if (!giftReceived) {
    test.skip(true, "선물함에 받을 수 있는 선물이 없어 토스트 메시지를 검증할 수 없습니다.");
  }
  await expect(page.getByText(/지급|이용권.*지급|선물.*완료/i).first()).toBeVisible({ timeout: 8000 });
});
